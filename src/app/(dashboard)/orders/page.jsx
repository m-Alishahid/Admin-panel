              ...
              {paginatedOrders.map((order) => (
                <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                    ${order.pricing?.grandTotal ? (order.pricing.grandTotal / 100).toFixed(2) : "0.00"}
                  </td>
                  <td className="px-6 py-4">
                    {getStatusBadge(order.status || "processed")}
                  </td>
                  <td className="px-6 py-4">
                    <button
                      onClick={() => openModal(order)}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-2">
                    <p><strong>Customer name:</strong> {selectedOrder.customer?.fullName || "Cameron Williamson"}</p>
                    <p><strong>Email:</strong> {selectedOrder.customerEmail || "cameronwilliamson@mail.com"}</p>
                    <p><strong>Phone:</strong> {selectedOrder.customerPhone || "(+1) 840-482-1485"}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <p><strong>Created at:</strong> {new Date(selectedOrder.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                    <p><strong>Delivery Services:</strong> Express</p>
                    <p><strong>Payment method:</strong> {selectedOrder.payment?.method?.toUpperCase() || "Bank Transfer"}</p>
                    <p><strong>Status:</strong> {getStatusBadge(selectedOrder.status || "Processed")}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">The order is being prepared (products are being packed)</p>
                    </div>
                  </div>
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Payment has been successfully processed and verified.</p>
                    </div>
                  </div>
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Order has been successfully placed by the customer</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span>${selectedOrder.pricing?.subtotal ? (selectedOrder.pricing.subtotal / 100).toFixed(2) : "120.99"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Shipping fee</span>
                    <span>${selectedOrder.shipping?.cost ? (selectedOrder.shipping.cost / 100).toFixed(2) : "5.75"}</span>
                  </div>
                  <div className="flex justify-between font-semibold text-lg border-t pt-2">
                    <span>Total</span>
                    <span>${selectedOrder.pricing?.grandTotal ? (selectedOrder.pricing.grandTotal / 100).toFixed(2) : "126.74"}</span>
                  </div>
                </div>
              </div>
            </div>
